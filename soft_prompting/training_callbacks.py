...
class TrainingCallbacks(ABC):
    ...
    def training_complete(self, model, model_name: str, dataset_name: str | None, tokenizer,
        ...
        :param dataset_name: The name of the dataset used for training, if any.
...
class SnapshottingCallbacks(TrainingCallbacks):
    ...
    def training_complete(self, model, model_name: str, dataset_name: str | None, tokenizer,
                          ...
                          soft_prompt: SoftPrompt, training_step_count: int, learning_rate: float,
                          weight_decay: float):
        try_create_snapshot(self.snapshot_path_creator, model_name, soft_prompt.soft_prompt_token_count, dataset_name,
    ...
    def training_complete(self, model, model_name: str, dataset_name: str | None, tokenizer,
        ...
        for i in range(0, len(self.prompts), batch_size):
            ...
            input_ids, output_ids = soft_prompting.training_and_testing.generate_from_prompts(
                ...
                None if self.soft_prompt_parameters is None else self.soft_prompt_parameters[start:end],
        ...
        if self.soft_prompt_parameters is None:
            formatted_results = [f"Sequence {i}:\n{seq}" for i, seq in enumerate(result_strings)]
        else:
        ...
        if self.results_path_creator is not None:
            results_path = self.results_path_creator(model_name, soft_prompt.soft_prompt_token_count, dataset_name)
